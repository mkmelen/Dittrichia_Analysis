...
```{r}
pldate <- mydata_dig %>% 
  select(Plant_ID, Treatment, PlantDate.L., PlantDate.R.) %>% 
...
plmeas<-mydata_dig %>% 
  select(Plant_ID,Treatment,LeftMeasure..mm.,RightMeasure..mm.) %>% 
...
plmort<-mydata_dig %>% 
  select(Plant_ID,Treatment,Mortality.Date..L.,Mortality.Date..R.) %>% 
  pivot_longer(cols=starts_with("Mortality"),names_to = "Position",values_to = "MortalityDate",names_prefix = "Mortality.Date..") %>% 
  mutate(Position=str_replace(Position,".$",""))

dig_work <- left_join(pldate, plmeas, by=c("Plant_ID","Treatment","Position")) %>% 
  left_join(., plmort, by=c("Plant_ID","Treatment","Position"))

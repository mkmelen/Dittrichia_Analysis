...
NEEDS:
Actually this graph doesn't show what Ingrid wants at all... take a look at the greenhouse photo
```{r}
...
comp_int_all_gg<-ggplot(data=mydata_all_fit_df,aes(x=factor(Soil),y=emmean,shape=Habitat,group=Habitat))+ylab("log(Biomass)")+xlab("Soil Type")+scale_y_continuous(limits=c(2.4,5.5),breaks=c(2.5,3,3.5,4,4.5,5,5.5))+theme_bw()+theme(panel.border=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(colour="black"))+geom_line(size=1,position=pd,color=c("gray58","gray58","forestgreen","forestgreen"))+geom_point(size=3,position=pd,color=c("gray58","gray58","forestgreen","forestgreen"))+geom_errorbar(color=c("gray58","gray58","forestgreen","forestgreen"),width=0.2,position=pd,aes(ymin=(emmean-SE),ymax=(emmean+SE)))

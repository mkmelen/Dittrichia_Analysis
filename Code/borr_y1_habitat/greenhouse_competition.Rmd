...
All data combined.
```{r}
mydata_fit<-lm(log_mydata$log_Dittrichia~Soil*Habitat,data=log_mydata)
x<-c("Soil","Habitat")
mydata_fit_df<-data.table(summary(emmeans(mydata_fit,specs=x)))
pd<-position_dodge(.2)
trueSE<-sd(log_mydata1$log_Dittrchia)/sqrt(length(log_mydata$log_Dittrichia))
mydata_Anova<-anova(mydata_fit)
mydata_Anova
...
comp_int_all_gg<-ggplot(
  ...
  geom_line(
    size=1,
    position=pd,
    color=c("orange","orange","purple","purple")
            )+
  geom_point(
    size=5,
    position=pd,
...
x <- c("Soil","Habitat")
mydata_fit_df <- data.table(summary(emmeans(mydata_fit,specs=x)))
#For all 16 iterations, summarize the data here but without emmeans... use log_mydata
#log_mydata%>%group_by(Habitat,Soil,Population)%>%summarise(Mean=mean(log_biomass),SD=sd(log_biomass),N=length(log_biomass))%>%mutate(SE=SD/sqrt(n))
pd<-position_dodge(.2)
trueSE <- sd(log_mydata1$log_Dittrchia)/sqrt(length(log_mydata$log_Dittrichia))
mydata_Anova<-anova(mydata_fit)
mydata_Anova

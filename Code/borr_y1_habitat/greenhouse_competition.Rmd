...
comp_int_all_gg<-ggplot(
  data=mydata_all_fit_df,
                aes(
                  x=factor(Soil),
                  y=emmean,
                  shape=Habitat,
  ...
  geom_line(
    size=1,
    position=pd,
    color=c("magenta","magenta","turquoise","turquoise")
            )+
  geom_point(
    size=5,
    position=pd,
...
#Assign legend to have the correct colors
```
Plot by site.
```{r}
...
pd<-position_dodge(.9)
comp_int_site_gg <- ggplot(
  data=mydata_site_sum,
                aes(
                  x=factor(Soil),
                  y=Mean,
                  color=Site,

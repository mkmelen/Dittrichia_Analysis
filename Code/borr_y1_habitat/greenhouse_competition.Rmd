...
Let's try putting both soil types together here.
```{r}
pd<-position_dodge(0)
...
comp_int_bio_all_gg<-ggplot(data=comp_int_bio_all,
                        aes(x=factor(Competition,levels=c("None","Festuca","Bromus")),
                            y=Mean))+
  scale_y_continuous(expression(italic("Dittrichia graveolens")~biomass ~""~(log(g10^4))),
                     limits=c(0,8), #y-axis: range
                     breaks=seq(0,8,1))+ #y-axis: incremental breaks by giving range and requesting a sequence by 1
  xlab("")+
  ggtitle("")+
  theme_classic(base_size=16)+
  theme(panel.border=element_blank(),
...
comp_scatter_gg<-ggplot(comp_scatter)+
  ...
  scale_x_continuous(expression("Grass"~biomass ~""~(log(g10^4))),

...
##Histograms
###Original data
The original data is skewed. So let's try log-transforming.
```{r}
#Dittrichia biomass
...
```
###Log transform data
Data is skewed, so let's try log transforming. However, the values are all <0 which create negative numbers when log transforming, therefore we need to multiply to move the decimal and then check the data again. And we find thatl it does not pass the Shapiro-Wilk normality test, therefore I'll try square-rooting the data.
...
sqrt_mydata<-sqrt(growth_mydata$Growth.Rate)
...
(CHE-O), 8 sites (random: population pairs; CHE), 2 habitat (fixed effect: roadside and off-road; R or O)

...
NEEDS:
Error bars! But will need to calculate based on original data with different sample sizes
```{r}
fakedata1<-data.frame(Habitat=c("Away","Roadside","Away","Roadside"),Treatment=c("Construction","Vegetated","Vegetated","Construction"),DaysToGerm=c(0),type=c("fake")) #make fake data to pull line back to 0

mydata1<-mydata%>%
    filter(DaysToGerm!=0)%>% 
  mutate(type=c("real"))%>%bind_rows(fakedata1) #add a column that differentiates real from fake data
  
germ_rate1<-mydata1%>% 
  mutate(PlotTreatment=paste(Habitat,Treatment, sep="/"))%>% 
  group_by(PlotTreatment,type)%>% 
  count(DaysToGerm)%>% 
  mutate(CumulativeSum=cumsum(n))%>% 
  mutate(Proportion=CumulativeSum/400) #calculating proportions using 400 seeds per group, 4 groups (2 Habitats x 2 Treatments)
  
germ_rate1_gg<-ggplot(germ_rate1,aes(x=DaysToGerm,y=Proportion,color=PlotTreatment,shape=PlotTreatment))+
  ...
  scale_color_manual(values=c("forestgreen","forestgreen","gray58","gray58"))+
  labs(y="Proportion Germinated",color="Habitat/Treatment",shape="Habitat/Treatment")
germ_rate1_gg
...
NEEDS:
Error bars! See comment above.
```{r}
fakedata2<-unique(mydata[c("Population","Treatment")])%>%
  mutate(type="fake",DaysToGerm=0)

mydata2<-mydata%>%
    filter(DaysToGerm!=0) %>% 
  mutate(type=c("real"))%>%bind_rows(fakedata2) #add a column that differentiates real from fake data
...
germ_rate2<-mydata2%>% 
  ...
  count(DaysToGerm)%>% 
  mutate(CumulativeSum=cumsum(n)) %>% 
  mutate(Proportion=CumulativeSum/50) #calculating proportions using 50 seeds per group

germ_rate2_gg<-ggplot(germ_rate2)+
  geom_line(aes(x=DaysToGerm,y=Proportion,color=PlotTreatment),size=0.5)+
  geom_point(aes(x=DaysToGerm,y=Proportion,color=PlotTreatment,shape=PlotTreatment,size=type))+
  theme_classic()+
  ylim(0,1)+
  scale_x_continuous(name="Days to Germination",limits=c(0,12),breaks=seq(0,12,1))+
  scale_size_manual(values=c(0,1.5),guide="none")+
  scale_shape_manual(values=c(1, 19, 1, 19, 0, 15, 0, 15, # 8 shape numbers in this array
                                ...
                                6, 25, 6, 25, 3, 12, 3, 12,
                                4, 13, 4, 13, 8, 11, 8, 11))+
  scale_color_manual(values=rep(c("forestgreen","forestgreen","gray58","gray58"),times=8))+
...
germ_int_all_gg<-ggplot(data=mydata_all_fit_df,
  ...
  scale_y_continuous(limits=c(2.1,2.6),
                     breaks=c(2.1,2.2,2.3,2.4,2.5,2.6))+
  theme_bw()+
  ...
  scale_color_manual(values=c("forestgreen","gray58"))+
  geom_errorbar(width=0.2,position=pd,
                aes(ymin=(emmean-SE),
                    ymax=(emmean+SE)))
germ_int_all_gg

ggsave(plot=germ_int_all_gg,file="/Users/Miranda/Documents/Education/UC Santa Cruz/Dittrichia/Dittrichia_Analysis/figures/germ_int_all_gg.png",width=25,height=20,units="cm",dpi=800)
```
...
mydata_site_sum<-mydata%>%
  ...
  summarise(Mean=mean(DaysToGerm),
            SD=sd(DaysToGerm),
            N=length(DaysToGerm))%>%
  mutate(SE=SD/sqrt(N))%>%
...
germ_int_site_gg<-ggplot(data=mydata_site_sum,
  ...
  geom_point(size=3,position=pd)+
  #scale_color_manual(values=c(1, 19, 1, 19, 0, 15, 0, 15, # 8 shape numbers in this array
  #                             2, 17, 2, 17, 5, 23, 5, 23,
  #                             6, 25, 6, 25, 3, 12, 3, 12,
  #                             4, 13, 4, 13, 8, 11, 8, 11))+
  #scale_color_manual(values=rep(c("forestgreen","forestgreen","gray58","gray58"),times=8))+
...
ggsave(plot=germ_int_site_gg,file="/Users/Miranda/Documents/Education/UC Santa Cruz/Dittrichia/Dittrichia_Analysis/figures/germ_int_site_gg.png",width=25,height=20,units="cm",dpi=800)
```

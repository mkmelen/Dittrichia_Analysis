...
mydata1<-mydata%>%
    filter(DaysToGerm!=0)%>% 
  mutate(type=c("real"))%>%bind_rows(fakedata1) #add a column that differentiates real from fake data so that data points only show on real data
...
```{r}
#NOT HELPFUL: This is actually days to germinate, and not proportion
...
germ_int_site_gg<-ggplot(data=mydata_site_sum,
  ...
  scale_shape_manual(values=c(19, 19, 15, 15,
                               ...
                               25, 25, 3, 3,
                               4, 4, 8, 8))+
  scale_color_manual(values=rep(c("forestgreen","gray58"),times=16))+
  labs(y="Proportion Germinated",color="Population/Habitat",shape="Population/Habitat")+

...
#ggplot
##Proportion Germinated x Time with Habitat and Treatment (all data pooled)
...
```{r}
#I have 160 dishes (DishNum) with 10 seeds germinating on different days, 4 treatments (PlotTreatment), so 40 reps for each PlotTreatment... the average of those 40 reps should be plotted with the errorbars using the global mean
...
germ_rate5<-data.frame(PlotTreatment="blank",
                      DaysToGerm=c(rep(seq(1,12,1),40)),
                      DishNum=rep(seq(1,160,1),each=12),
                      PropGerm=0,CumPropGerm=0)%>% #CANNOT DO THIS!!! WRONG TO HAVE THESE PIPE TOGETHER
  filter(germ_rate5$DishNum%in%germ_rate4$DishNum) #actually prefilter dish numbers

#for loop to fill in missing data, may take a bit, warnings are just about making empty data frame
for (i in 1:nrow(germ_rate5)){
  #first check if the dish number is in the data set
  if(germ_rate5$DishNum[i]%in%germ_rate4$DishNum){
    #make temporary data frame of the data set of dish
    temp<-germ_rate4%>%
      filter(DishNum==germ_rate5$DishNum[i])%>%#first filter for the right dish
      mutate(diff=DaysToGerm-as.numeric(germ_rate5$DaysToGerm[i]))
    ...
    if(nrow(filltemp)>0){#check if it is empty data frame, meaning no close day
      germ_rate5[i,]<-c(filltemp$PlotTreatment,germ_rate5$DaysToGerm[i],germ_rate5$DishNum[i],filltemp$PropGerm,filltemp$CumPropGerm)
    }else{
    germ_rate5[i,]<-c(temp$PlotTreatment[1],germ_rate5$DaysToGerm[i],germ_rate5$DishNum[i],0,0)
  }}
...
#we want the graph to start at zero, but the data starts on day 2, so here we create fake data for the line to use
fakedata3<-data.frame(PlotTreatment=c("Roadside/Construction Soil","Roadside/Field Soil","Vegetated/Construction Soil","Vegetated/Field Soil"),
                      DaysToGerm=c(0),
                      DishNum=c(0),
                      PropGerm=c(0),
                      CumPropGerm=c(0),
...
fakedata3$DaysToGerm<-as.character(fakedata3$DaysToGerm)
fakedata3$DishNum<-as.character(fakedata3$DishNum)
fakedata3$PropGerm<-as.character(fakedata3$PropGerm) 
fakedata3$CumPropGerm<-as.character(fakedata3$CumPropGerm)
fakedata3$type<-as.character(fakedata3$type) 

#put real data and fake data together
germ_rate6<-germ_rate5%>%
  mutate(type=c("real"))%>% #here we add a column to identify our real data
  bind_rows(fakedata3) #now we add our fake data so that we have one dataframe with real and fake data (labeled, of course)

#make a summary table  
germ_rate7<-germ_rate6%>% 
...
germ_rate7_gg<-ggplot(germ_rate7, #making a graph with ggplot
                      aes(x=DaysToGerm, #variable for x-axis
                          y=CumPropGerm, #variable for y-axis
                          color=PlotTreatment, #how colors should be assigned
                          shape=PlotTreatment))+ #how shapes should be assigned
  geom_line(linewidth=0.5)+ #now we make the line
  geom_point(aes(size=type))+ #now we add the data points
  #geom_errorbar( #and here are the error bars!
#                aes(ymin=mean_CumPropGerm-se_prop,
...
ggsave(plot=germ_rate7_gg,file="/Users/Miranda/Documents/Education/UC Santa Cruz/Dittrichia/Dittrichia_Analysis/figures/germ_rate7_gg.png",width=25,height=15,units="cm",dpi=800)
```
...
germ_int_all_gg<-ggplot(data=germ_rate3,
...
ggsave(plot=germ_int_all_gg,file="/Users/Miranda/Documents/Education/UC Santa Cruz/Dittrichia/Dittrichia_Analysis/figures/germ_int_all_gg.png",width=25,height=15,units="cm",dpi=800)
```

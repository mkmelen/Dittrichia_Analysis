...
#By soil type
ggplot(mydata,aes(x=DaysToGerm))+geom_histogram()+facet_wrap(vars(DishType)) #Here we see that Dittrichia germination responds similarly to both soil types (DishType).
...
#1.041815        1.002255 
#Both are just above 1, which means that roadside has an ever so so slight increase in germination compared to off-road, and forest has an even smaller slight increase in germination compared to construction.
...
exp(ranef(fullmodel2)$Site/Population/DishNum) # Pretty good for most, G and J are lower
